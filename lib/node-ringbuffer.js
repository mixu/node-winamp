 ...
 * 
 * This utility class makes manipulating streams which return buffers - such as TCP
 * client connections - simpler. 
 * 
 * The ring buffer exposes a simple interface:
 * - You add() buffers to it -- until the maximum capacity is reached. 
 *   When that happens, the old buffer contents get overwritten (oldest content first).
 *   
 * - When you want to manipulate the data, you retrieve it as a string via toString().
 *   The fact that the data is stored in a ring buffer is transparent to the end-user.
 *   You can then use the regular string manipulation operations.
 *   
 * - You can skip forward using skip() after you are done with the data. 
 *   This is high-performance, since none of the data in the ring buffer is touched.   
 *   The skip() function can forward further than it has data. This is useful if you
 *   need to skip some data in a stream - for example, in a stream that mixes string and 
 *   binary data.
 *
 */
